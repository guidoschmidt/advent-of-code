...
fn printMap(map: *[][]u8) void {
    ...
    for (0..map.len) |x| {
        const row = map.*[x];
        for (0..row.len) |y| {
            std.debug.print("{c}{s}", .{ map.*[x][y], aoc.clear });
        }
        std.debug.print("\n", .{});
    }
}
...
fn floodFill(gpa: Allocator, map: *[][]u8, start: [2]usize) !void {
    var stack = std.array_list.Managed([2]usize).init(gpa);
    defer stack.deinit();
...
fn part1(gpa: Allocator, input: []const u8) anyerror!void {
    ...
    var command_list = std.array_list.Managed(Command).init(gpa);
    while (row_it.next()) |row| {
        var part_it = std.mem.tokenizeSequence(u8, row, " ");
        const dir: Dir = switch (part_it.next().?[0]) {
            'U' => Dir.U,
            'D' => Dir.D,
            'L' => Dir.L,
            'R' => Dir.R,
        ...
        const meters = try std.fmt.parseInt(u16, part_it.next().?, 10);
...
fn part2(gpa: Allocator, input: []const u8) anyerror!void {
    ...
    var corners = std.array_list.Managed([2]i128).init(gpa);
    ...
    const scaler = 1000;
    try svg.init("map.svg", @intCast(@divFloor(vp_width, scaler)), @intCast(@divFloor(vp_height, scaler)), @intCast(@divFloor(min[0], scaler)), @intCast(@divFloor(min[1], scaler)), @intCast(@divFloor(max[0], scaler)), @intCast(@divFloor(max[1], scaler)));
    try svg.startPolygon();
    for (0..corners.items.len) |i| {
        const p = corners.items[i];
        try svg.addPolygonPoint(@intCast(@divFloor(p[0] - min[0], scaler)), @intCast(@divFloor(p[1] - min[1], scaler)));
    }
    try svg.endPolygon();
    try svg.close();
    ...
    const shoe_lace_sum = try shoeLace(&corners.items) + @divExact(outline_sum, 2) + 1;
...
pub fn main() !void {
    ...
    try aoc.runPart(gpa, 2023, 18, .PUZZLE, part1);
    try aoc.runPart(gpa, 2023, 18, .PUZZLE, part2);
}
