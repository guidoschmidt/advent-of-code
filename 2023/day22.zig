...
const Brick = struct {
    ...
    pub fn format(self: Brick, comptime fmt: []const u8, options: std.fmt.FormatOptions, writer: anytype) !void {
        _ = fmt;
        _ = options;
        try writer.print("\n[{d}, {d}, {d}] - [{d}, {d}, {d}]", .{ self.a[0], self.a[1], self.a[2], self.b[0], self.b[1], self.b[2] });
    }
};

fn part1(gpa: Allocator, input: []const u8) anyerror!void {
    var brick_list = std.array_list.Managed(Brick).init(gpa);

    var row_it = std.mem.tokenizeSequence(u8, input, "\n");
    while (row_it.next()) |row| {
        var pos_it = std.mem.tokenizeSequence(u8, row, "~");
        const start_str = pos_it.next().?;
        var start_it = std.mem.tokenizeSequence(u8, start_str, ",");
        ...
        var end_it = std.mem.tokenizeSequence(u8, end_str, ",");
