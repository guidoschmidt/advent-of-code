...
fn listToNumber(list: std.array_list.Managed(u8)) u32 {
    var number: u32 = 0;
...
fn expand(allocator: Allocator, data: *[]u8, x: i16, y: i16, x_off: i16, y_off: i16, column_count: i16) !u32 {
    var left_numbers = std.array_list.Managed(u8).init(allocator);
    var right_numbers = std.array_list.Managed(u8).init(allocator);
...
fn part2(allocator: Allocator, input: []const u8) anyerror!void {
    ...
    for (0..col_count) |x| {
        for (0..row_count) |y| {
            ...
            var gears = std.array_list.Managed(u32).init(allocator);
            defer gears.deinit();
            for (0..3) |xo| {
                ...
                for (0..3) |yo| {
                    ...
                    const x_o: i16 = @as(i16, @intCast(xo)) - 1;
                    if (x_o == 0 and y_o == 0) continue;
                    const idx_off = indexWithOffset(@intCast(x), @intCast(y), @intCast(col_count), x_o, y_o, clean_data.len);
                    ...
                    _ = digit;
                    part_num_count += 1;
                    const number = try expand(allocator, &clean_data, @as(i16, @intCast(x)), @as(i16, @intCast(y)), x_o, y_o, @intCast(col_count));
            ...
            std.debug.print("\n  → {d} Gears: {any}", .{ part_num_count, gears.items });
...
pub fn main() !void {
    ...
    try aoc.runPart(allocator, 2023, 3, .PUZZLE, part1);
    try aoc.runPart(allocator, 2023, 3, .PUZZLE, part2);
}
