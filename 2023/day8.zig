...
const LR = struct { l: []const u8, r: []const u8 };
...
fn parse(input: []const u8, nodes_map: *std.StringHashMap(LR), lr_instructions: *std.ArrayList(u8)) void {
    ...
    while (row_it.next()) |row| {
        ...
        var lr_values_it = std.mem.splitSequence(u8, r_side[1 .. r_side.len - 1], ",");
        const l_value = std.mem.trim(u8, lr_values_it.next().?, " ");
        const r_value = std.mem.trim(u8, lr_values_it.next().?, " ");
...
fn part2(allocator: Allocator, input: []const u8) anyerror!void {
    ...
    for (0..positions.items.len) |i| {
        ...
        walkers.append(Walker{ .pos = @constCast(pos), .counter = 0, .instr_idx = 0 }) catch {
            std.log.err("\nERROR: could not append to walkers", .{});
        };
        const t = std.Thread.spawn(.{}, walk, .{ &walkers.items[i], &nodes_map, &lr_instructions }) catch undefined;
        t.join();
    }

    // Calculate least aoc multiple
...
pub fn main() !void {
    ...
    try aoc.runPart(allocator, 2023, 8, .PUZZLE, part1);
    try aoc.runPart(allocator, 2023, 8, .PUZZLE, part2);
}
