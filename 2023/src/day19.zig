...
const PartRating = struct {
    ...
    pub fn format(self: PartRating, comptime fmt: []const u8, options: std.fmt.FormatOptions, writer: anytype) !void {
        _ = fmt;
        _ = options;
...
const Workflow = struct {
    ...
    pub fn format(self: Workflow, comptime fmt: []const u8, options: std.fmt.FormatOptions, writer: anytype) !void {
        _ = fmt;
        _ = options;
        try writer.print("[{s}]\nRules:", .{self.name});
        for (self.rules) |rule| {
