...
fn part1(allocator: Allocator, input: []const u8) anyerror!void {
    var times = std.array_list.Managed(u32).init(allocator);
    var dists = std.array_list.Managed(u32).init(allocator);

    var row_it = std.mem.tokenizeAny(u8, input, "\n");
    while (row_it.next()) |r| {
        var val_it = std.mem.tokenizeAny(u8, r, " ");
...
fn part2(allocator: Allocator, input: []const u8) anyerror!void {
    ...
    var time_it = std.mem.tokenizeAny(u8, time_row, ":");
    _ = time_it.next().?;
    ...
    var dist_it = std.mem.tokenizeAny(u8, dist_row, ":");
    _ = dist_it.next().?;
