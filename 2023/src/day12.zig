...
fn countCombinationsMemoizedNew(allocator: Allocator, springs: []const u8, groups: []const usize) !usize {
    ...
    for (1..springs.len + 1) |s| {
        ...
        // std.debug.print("\n{s}LOOP {c}{s}", .{ aoc.green, spring, aoc.clear });
        ...
        for (0..groups.len + 1) |g| {
            ...
            if (spring == '.' or spring == '?') {
                ...
                count += prev;
                // std.debug.print("\nCASE 1 {s}no group:{s} {d}, [+{d}]", .{ aoc.light_blue, aoc.clear, count, prev });
            }

            // Groups
            if (g > 0) {
                const group_size = groups[g - 1];
                // std.debug.print("\nCASE 2 {s}group of size {d}{s}", .{ aoc.light_blue, group_size, aoc.clear });
                const partial = springs[(s -| 1) -| (group_size -| 1)..s];
                // std.debug.print("\nPartial: {s}{s}{s}", .{ aoc.yellow, partial, aoc.clear });
                var test_all = true;
                for (partial) |item| {
                ...
                if (s >= group_size) {
                    // std.debug.print("\nCASE 3 {s}s={d} >= group_size={d}{s}", .{ aoc.light_blue, s, group_size, aoc.clear });
                    if (test_all) {
                        if ((s) == group_size) {
                            // std.debug.print("\nCASE 3.1 {s}s={d} == group_size={d}{s}", .{ aoc.light_blue, s, group_size, aoc.clear });
                            const prev = cache[0][g - 1];
                            count += prev;
                            // std.debug.print("{d} [+{d}]", .{ count, prev });
                        } else {
                            // std.debug.print("\nCASE 4 {s}group_size={d} > {d}{s}", .{ aoc.light_blue, group_size, g, aoc.clear });
            ...
            // std.debug.print("\n    {s}---> {d}{s}", .{ aoc.blue, count, aoc.clear });
            cache[s][g] = count;
        }
        // std.debug.print("\n", .{});
    }
...
pub fn main() !void {
    ...
    try aoc.runPart(allocator, 2023, 12, .PUZZLE, part1);
    try aoc.runPart(allocator, 2023, 12, .PUZZLE, part2);
}
