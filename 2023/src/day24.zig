...
fn part1(gpa: Allocator, input: []const u8) anyerror!void {
    ...
    for (0..hailstones.items.len) |idx_a| {
        ...
        for (idx_a..hailstones.items.len) |idx_b| {
            ...
            const end_min_b = hailstone_b.findEnd(MIN);
            const end_max_b = hailstone_b.findEnd(MAX);
...
pub fn main() !void {
    ...
    try aoc.runPart(gpa, 2023, 24, .PUZZLE, part1);
    try aoc.runPart(gpa, 2023, 24, .PUZZLE, part2);
}
